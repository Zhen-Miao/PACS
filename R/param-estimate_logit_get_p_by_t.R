...
loss_grad_pen <- function(xdumm, p_bg, q_vec, inf_mat, wii_sqrt_X) {
  ...
  i_infor_m <- try(solve(inf_mat), silent = T)
...
irls_iter_null <- function(y_vec, xdumm, theta_estimated,hold_zero, stop_criteria = 1e-6, q_vec) {
  indi <- 1
  n_iter <- 1
  conv.stat <- 1 ## 1 means converge, 2 means matrix singular, 3 means reaches max iter
  # y_vec = as.vector(y_vec)
  poi = setdiff(c(1:length(theta_estimated)), hold_zero)
...
## Iteratively reweighted least squares function -- newton method (main function)
#' Title
#'
#' @param xdumm
#' @param theta_estimated
...
estimate_parameters <- function(
  ...
  r_by_c <- as.data.frame(t(r_by_c)) ## now it is cell by region matrix

  tic('parameter_estimation')
  print('running parameter estimation')
  theta_estimated_one_list <- mclapply(r_by_c, FUN = irls_iter,
  ...
  if(sum(fail_converge) == 0){
    ...
    r_by_c <- as.data.frame(r_by_c[,fc_regions])

    par_initial = -1*par_initial
    tic('newton method for estimation')
    theta_estimated_one_list_nt <- mclapply(r_by_c, FUN = irls_iter_nt,
                                            xdumm = design_mat, theta_estimated = par_initial,
                                            q_vec = cap_rate_vec, mc.cores = mc.cores)
    toc()
...
estimate_parameters_null <- function(
  ...
  tic('parameter_estimation')
  print('running parameter estimation')
  theta_estimated_one_list <- mclapply(r_by_c, FUN = irls_iter_null,
                                       xdumm = design_mat, theta_estimated = par_initial,
                                       hold_zero = hold_zero,
                                       q_vec = cap_rate_vec, mc.cores = mc.cores)
  print('running parameter estimation')
  ...
  if(sum(fail_converge) == 0){
    ...
    par_initial = -1*par_initial
    tic('newton method for estimation')
    theta_estimated_one_list_nt <- mclapply(r_by_c, FUN = irls_iter_nt_null,
                                            xdumm = design_mat, theta_estimated = par_initial,
                                            hold_zero = hold_zero,
                                            q_vec = cap_rate_vec, mc.cores = mc.cores)
    toc()
