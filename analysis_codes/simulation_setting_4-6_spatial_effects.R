...
lam_list = readRDS('data/lam_list_low_23.rds') ## list of insertion rate lambda for each cell type
...
for(iii in 1:n_repeat){
  ...
  our_p = pacs_test_sparse(
     ...
     formula_null = ~ factor(group),
     pic_matrix = data_mat,
     n_peaks_per_round = NULL,
     T_proportion_cutoff = 0.4,
     cap_rates = cap_rates
   )
 ...
 seurat_x_dummy_null = cbind(xdummy_null, n_reads_cell)
 p_value_mat[['Seurat']][,iii] =
   seurat_method3_other_factors(data_mat,seurat_x_dummy_null,
                                group_info = batch.info, batch_info = group.info)

 # archR method -- marginal
 p_value_mat[['archR_n']][,iii] = archR_method(cbind(data_matrix_t1b1, data_matrix_t2b1),
                                                      cbind(data_matrix_t1b2, data_matrix_t2b2))

 # archR method -- stratified
 p_arch_s1 = archR_method(data_matrix_t1b1, data_matrix_t1b2)
 p_arch_s2 = archR_method(data_matrix_t2b1, data_matrix_t2b2)
 ...
 # snapATAC_marginal
 p_value_mat[['snapATAC_n']][,iii] = snapATAC_method(cbind(data_matrix_t1b1, data_matrix_t2b1),
                                                     cbind(data_matrix_t1b2, data_matrix_t2b2), bcv = 0.4)

 # snapATAC method -- stratified
 p_sn_s1 = snapATAC_method(data_matrix_t1b1, data_matrix_t1b2, bcv = 0.4)
 p_sn_s2 = snapATAC_method(data_matrix_t2b1, data_matrix_t2b2, bcv = 0.4)
 ...
 # Fisher
 p_value_mat[['Fisher_n']][,iii] = fisher_method(cbind(data_matrix_t1b1, data_matrix_t2b1),
                                                 cbind(data_matrix_t1b2, data_matrix_t2b2))

 # Fisher method -- stratified
 p_fi_s1 = fisher_method(data_matrix_t1b1, data_matrix_t1b2)
 p_fi_s2 = fisher_method(data_matrix_t2b1, data_matrix_t2b2)
 ...
 p_value_mat[['Fisher_s']][,iii] = apply(X = p_mat_fi_s,1, FUN = function(x) sump(p = x)$p)
  ...
  mat_combined = cbind(data_matrix_t1b1, data_matrix_t1b2,
                       data_matrix_t2b1, data_matrix_t2b2)
  ## edgeR-multi method
  group.info <- c(rep.int(0,times = n_cell_sample_t1b1 + n_cell_sample_t1b2),
                  rep.int(1,times = n_cell_sample_t2b1 + n_cell_sample_t2b2 ))
  ...
  pseudo_t1_b1 <- create_pseudo_bulk(data_matrix_t1b1, 100)
  pseudo_t1_b2 <- create_pseudo_bulk(data_matrix_t1b2, 100)
  pseudo_t2_b1 <- create_pseudo_bulk(data_matrix_t2b1, 100)
  pseudo_t2_b2 <- create_pseudo_bulk(data_matrix_t2b2, 100)
...
saveRDS(p_value_mat, 'results/multifactor_simulation_marginal_p_value_mat_spatial_effect very unbalanced edgeR only 100 May 2.rds')
...
methods_all <- c('PACS', 'Seurat', 'Logistic', 'archR_n','archR_s', 'edgeR_pseudo_n', 'edgeR_n',
                 'snapATAC_n','snapATAC_s' ,'Fisher_n', 'Fisher_s')
# methods_all <- c('edgeR_pseudo_n', 'edgeR_n')
...
### it is a different rule to determine t1e and power now

### for 4-6
for(jj in methods_all){
  t1e_v <- colMeans(p_value_mat[[jj]][c(1:1000, 3001:4000),] < 0.05, na.rm = T)
  t1power_mat[jj,'t1e'] <- mean(t1e_v,na.rm = T)
  t1power_mat[jj,'t1e_sd'] <- sd(t1e_v,na.rm = T)
  power_v <- colMeans(p_value_mat[[jj]][c(1001:3000, 4001:6000),] < 0.05, na.rm = T)
...
saveRDS(t1power_mat,'results/t1power_mat_marginal_spatial_constant_effect_size_very_unbalanced_May2 edgeR only 100.rds')
