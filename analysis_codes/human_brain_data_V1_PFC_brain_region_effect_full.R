...
brain = readRDS('brain_mat_PFC_V1.rds')
...
full_para = estimate_parameters(r_by_c = brain,
               design_mat = xdummy,
               par_initial = par_initial_one,
               cap_rate_vec = r_by_ct$q_vec_new,mc.cores = 1)

saveRDS(full_para, paste0('full_para_', iii, '.rds'))
